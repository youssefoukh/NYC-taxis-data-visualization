! function(name, context, definition) {
    ...
    'use strict';
    var defaults = { autoPlay: false, autoPlayDelay: 1000, data: [], enableLabelClick: true, enableMarkerClick: true, enableTrackClick: true, handlers: [], highlightSelectedLabels: false, labelsPosition: "below", labelsWidth: 60, leftLabel: "", loop: true, markerSize: 14, rightLabel: "", showLabels: false, sideLabelsWidth: 40, showTrackMarkersProgress: false, showTicks: true, showTrackMarkers: false, showTrackProgress: false, showValue: false, startAlternateLabelsFromTop: false, startPosition: 0, stepSize: 1, thumbHeight: 16, thumbWidth: 16, tickHeight: 16, trackHeight: 7, valueIndicatorOffset: 5, valueIndicatorWidth: 30, valueIndicatorHeight: 30, valuePosition: "thumb", valueSource: "index" };

    function Rangeslide() { this.init.apply(this, arguments) };
    Rangeslide.prototype = {
        ...
        checkIfTargetIsValid: function(target) {
            if (!target) { throw new Error("Missing required attribute: target element (css selector or DOM node) must be provided"); } else if (this.__isString(target)) { this.__targetElement = document.querySelector(target); } else if (this.__isDOMElement(target)) { this.__targetElement = target; } else { throw new Error("Incorrect type: target element must be DOM node or string css selector"); }
        },
        createUI: function() {
            ...
            if (this.config.showTrackMarkers) { this.__trackMarkerElements = this.createTrackMarkers(); var l = this.__trackMarkerElements.length; while (l--) { this.__trackElement.appendChild(this.__trackMarkerElements[l]); } }
            return fragment;
        },
        createTrackElement: function() {
            ...
            if (this.config.enableTrackClick) { trackElement.onclick = this.onTrackClicked; }
            return trackElement;
        },
        ...
        createLabels: function(parent, everyOtherLabel, skipFirst, labelsBeforeTicks) {
            ...
            for (var i = skipFirst ? 1 : 0; i <= total; everyOtherLabel ? i += 2 : i++) {
                ...
                if (this.config.enableLabelClick) { labelElement.onclick = this.onLabelClicked; }
                ...
                if (labelsBeforeTicks) { labelElement.insertBefore(span, tickElement); } else { labelElement.appendChild(span); }
                parent.appendChild(labelElement);
            }
            if (labelsBeforeTicks) { parent.style.height = 14 + (this.config.showTicks ? this.config.tickHeight : 0) + "px"; }
        },
        ...
        getElement: function() { return this.__targetElement; },
        getThumbStart: function() { return isNaN(parseFloat(this.__thumbElement.style.left)) ? 0 : parseFloat(this.__thumbElement.style.left); },
        ...
        goToNextMarker: function() {
            ...
            if (this.config.loop) { if (nextMarker > this.config.data.length - 1) { nextMarker = 0; } } else { if (nextMarker > this.config.data.length - 1) { clearInterval(this.__playInterval); return; } }
        ...
        snap: function(positionX) {
            ...
            if (this.config.showTrackProgress) { this.__trackProgressElement.style.width = newLeft + "px"; }
            if (this.config.showTrackMarkersProgress) { this.__markersUpdateInterval = setInterval(this.onUpdateMarkersProgress, 50); }
        ...
        setActiveLabel: function(index) {
            ...
            if (previouslyActiveLabel) { previouslyActiveLabel.classList.remove("selected"); }
            if (currentlyActiveLabel) { currentlyActiveLabel.classList.add("selected"); }
        },
        onValueChanged: function(previousValue, currentValue) {
            if (this.config.showValue) { this.__valueElement.innerText = currentValue[this.config.valueSource]; }
            if (this.config.highlightSelectedLabels) { this.setActiveLabel(currentValue.id); }
        ...
        onThumbMouseMove: function(event) {
            ...
            if (!this.isPositionOutOfBounds(newLeft)) { this.__thumbElement.style.left = newLeft + 'px'; }
        ...
        onThumbTransitionEnd: function(event) {
            ...
            if (this.config.showTrackMarkersProgress) { this.onUpdateMarkersProgress(); }
            clearInterval(this.__markersUpdateInterval);
        },
        onUpdateMarkersProgress: function() {
            var l = this.__trackMarkerElements.length;
            while (l--) {
                if (this.__trackMarkerElements[l].offsetLeft <= this.__thumbElement.offsetLeft) { this.__trackMarkerElements[l].classList.add("completed"); } else { this.__trackMarkerElements[l].classList.remove("completed"); }
            }
        },
        isPositionOutOfBounds: function(position) { var size = this.config.data.length - 1; var distanceBetweenLabels = (this.__targetElement.clientWidth - this.config.thumbWidth) / size; return (position > distanceBetweenLabels * size) || (position < 0); },
        getMarkerByIndex: function(index) { return { index: index, name: this.config.data[index].name, item: this.config.data[index].item }; },
        getMarkerByName: function(name) { for (var i = 0; i < this.config.data.length; i++) { if (this.config.data[i].name === name) { return { index: i, name: this.config.data[i].name, item: this.config.data[i].item }; } } },
        fire: function(eventName, args) {
            if (!this.config.handlers || !this.config.handlers[eventName]) { return; }
            var l = this.config.handlers[eventName].length;
            while (l--) { this.config.handlers[eventName][l].apply(this, args); }
        },
        __isDOMElement: function(target) { return target instanceof Element; },
        __isString: function(target) { return typeof target === "string"; }
    };

    function rangeslide(target, config) { return new Rangeslide(target, config); }
    return rangeslide;
});